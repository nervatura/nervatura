...
func TestGService_ProjectUpdate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				req: &pb.Project{Id: 1, Code: "1234", ProjectName: "Test Project", CustomerCode: "1234",
					Contacts: []*pb.Contact{
						{FirstName: "hello"},
					}},
				ds: &api.DataStore{
		...
		},
		{
			...
			args: args{
				ctx:  context.Background(),
				req:  &pb.Project{Id: 0, Code: "1234", ProjectName: "Test Project", CustomerCode: "1234"},
				user: md.Auth{UserGroup: md.UserGroupAdmin},
				ds: &api.DataStore{
					AppLog: slog.New(slog.NewTextHandler(bytes.NewBufferString(""), nil)),
					Db: &td.TestDriver{
		...
		},
		{
			...
			args: args{
				ctx:  context.Background(),
				req:  &pb.Project{Id: 1, Code: "1234", CustomerCode: "1234"},
				user: md.Auth{UserGroup: md.UserGroupAdmin},
			},
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := s.ProjectUpdate(ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GService.ProjectUpdate() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
...
func TestGService_ProjectQuery(t *testing.T) {
	type fields struct {
		Config                 cu.IM
		AppLog                 *slog.Logger
		DecodeAuthToken        func(tokenString string) (cu.IM, error)
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				req: &pb.RequestQuery{Limit: 10, Offset: 0, Filters: []*pb.RequestQueryFilter{
					{Field: "project_name", Value: "Test Project"},
				}},
				ds: &api.DataStore{
					AppLog: slog.New(slog.NewTextHandler(bytes.NewBufferString(""), nil)),
					Db: &td.TestDriver{
		...
		},
		{
			...
			args: args{
				ctx: context.Background(),
				req: &pb.RequestQuery{Limit: 10, Offset: 0, Filters: []*pb.RequestQueryFilter{
					{Field: "invalid", Value: "Test Project"},
				}},
			},
			wantErr: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), md.DataStoreCtxKey, tt.args.ds)
			_, err := s.ProjectQuery(ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GService.ProjectQuery() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
