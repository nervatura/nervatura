...
func TestGService_CurrencyUpdate(t *testing.T) {
	type fields struct {
		Config                 cu.IM
		AppLog                 *slog.Logger
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				req: &pb.Currency{Id: 0, Code: "123456"},
				ds: &api.DataStore{
					AppLog: slog.New(slog.NewTextHandler(bytes.NewBufferString(""), nil)),
					Db: &td.TestDriver{
						Config: cu.IM{
							"Query": func(queries []md.Query) ([]cu.IM, error) {
								return []cu.IM{{"id": 1, "currency_object": `{"id": 1, "code": "123456"}`}}, nil
							},
		...
		},
		{
			name: "get data error",
			args: args{
				req: &pb.Currency{Id: 1, Code: "123456"},
				ds: &api.DataStore{
					AppLog: slog.New(slog.NewTextHandler(bytes.NewBufferString(""), nil)),
					Db: &td.TestDriver{
						Config: cu.IM{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := s.CurrencyUpdate(ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GService.CurrencyUpdate() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
...
func TestGService_CurrencyGet(t *testing.T) {
	type fields struct {
		Config                 cu.IM
		AppLog                 *slog.Logger
		DecodeAuthToken        func(tokenString string) (cu.IM, error)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), md.DataStoreCtxKey, tt.args.ds)
			_, err := s.CurrencyGet(ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GService.CurrencyGet() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
...
func TestGService_CurrencyQuery(t *testing.T) {
	type fields struct {
		Config                 cu.IM
		AppLog                 *slog.Logger
		DecodeAuthToken        func(tokenString string) (cu.IM, error)
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				ctx: context.Background(),
				req: &pb.RequestQuery{Limit: 10, Offset: 0, Filters: []*pb.RequestQueryFilter{{Field: "tag", Value: "1"}}},
				ds: &api.DataStore{
					AppLog: slog.New(slog.NewTextHandler(bytes.NewBufferString(""), nil)),
					Db: &td.TestDriver{
						Config: cu.IM{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ctx := context.WithValue(context.Background(), md.DataStoreCtxKey, tt.args.ds)
			_, err := s.CurrencyQuery(ctx, tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GService.CurrencyQuery() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
		})
	}
