...
func init() {
	toolDataMap["nervatura_worksheet_create"] = McpTool{
		Tool: mcp.Tool{
			Name:        "nervatura_worksheet_create",
			Title:       "Create a new worksheet",
			Description: "Create a new worksheet. Related tools: item.",
			Meta: mcp.Meta{
				"scopes": []string{"worksheet"},
			},
		},
		ModelSchema: WorksheetSchema(),
		ConnectHandler: func(server *mcp.Server, tool *mcp.Tool) {
			mcp.AddTool(server, tool, worksheetCreateHandler)
		},
	}
	toolDataMap["nervatura_worksheet_query"] = McpTool{
		Tool: mcp.Tool{
			Name:        "nervatura_worksheet_query",
			Title:       "Query worksheets by parameters",
			Description: "Query worksheets by parameters. The result is all worksheets that match the filter criteria.",
			Meta: mcp.Meta{
				"scopes": []string{"worksheet"},
			},
		},
	...
	toolDataMap["nervatura_worksheet_update"] = McpTool{
		Tool: mcp.Tool{
			Name:        "nervatura_worksheet_update",
			Title:       "Update a worksheet by code",
			Description: "Update a worksheet by code. When modifying, only the specified values change. Related tools: item.",
			Meta: mcp.Meta{
				"scopes": []string{"worksheet"},
			},
		},
	...
	toolDataMap["nervatura_worksheet_delete"] = McpTool{
		Tool:        createDeleteTool("nervatura_worksheet_delete", "worksheet", mcp.Meta{"scopes": []string{"worksheet"}}),
		ModelSchema: WorksheetSchema(),
		ConnectHandler: func(server *mcp.Server, tool *mcp.Tool) {
...
type worksheetCreate struct {
	TransDate    string `json:"trans_date" jsonschema:"Start date. Required when creating a new worksheet. Example: 2025-01-01"`
	Direction    string `json:"direction" jsonschema:"Transaction direction. Enum values. Required when creating a new worksheet. Example: DIRECTION_OUT"`
	...
	CustomerCode string `json:"customer_code" jsonschema:"Customer reference. Required when creating a new worksheet. Example: CUS1731101982N123"`
	...
	CurrencyCode string `json:"currency_code" jsonschema:"Currency iso code. Required when creating a new worksheet. Example: USD"`
	worksheetMeta
...
type worksheetUpdate struct {
	Code         string `json:"code" jsonschema:"Database independent unique key. Required when updating an existing worksheet."`
	TransDate    string `json:"trans_date,omitempty" jsonschema:"Start date."`
...
type worksheetMeta struct {
	DueTime       string                `json:"due_time,omitempty" jsonschema:"End date. Required when creating a new worksheet. Example: 2025-01-01"`
	...
	Closed        bool                  `json:"closed,omitempty" jsonschema:"Closed worksheet"`
	...
	Worksheet     md.TransMetaWorksheet `json:"worksheet" jsonschema:"Worksheet metadata. Required all fields."`
}
...
type worksheetQuery struct {
	...
	TransDate    string        `json:"trans_date,omitempty" jsonschema:"Worksheet date."`
	Direction    string        `json:"direction,omitempty" jsonschema:"Transaction direction. Enum values. Example: DIRECTION_OUT"`
	TransCode    string        `json:"trans_code,omitempty" jsonschema:"Other transaction (invoice, receipt, offer, worksheet, worksheet, rent etc.) reference. Optional. Example: ORD1731101982N123"`
	CustomerCode string        `json:"customer_code,omitempty" jsonschema:"Customer reference. Example: CUS1731101982N123"`
	EmployeeCode string        `json:"employee_code,omitempty" jsonschema:"Employee reference. Example: EMP1731101982N123"`
	ProjectCode  string        `json:"project_code,omitempty" jsonschema:"Project reference. Example: PRJ1731101982N123"`
	CurrencyCode string        `json:"currency_code,omitempty" jsonschema:"Currency iso code. Example: USD"`
	Amount       float64       `json:"amount,omitempty" jsonschema:"Total amount."`
	TransMeta    worksheetMeta `json:"trans_meta,omitempty" jsonschema:"Trans metadata."`
	TransMap     cu.IM         `json:"trans_map,omitempty" jsonschema:"Flexible key-value map for additional metadata. The value is any json type."`
}

type worksheetParameter struct {
	Code         string `json:"code,omitempty" jsonschema:"Database independent unique key."`
...
func WorksheetSchema() (ms *ModelSchema) {
	return &ModelSchema{
		Name:       "trans",
		...
		CustomParameters: func(params cu.IM) cu.IM {
			params["filter"] = "trans_type = '" + md.TransTypeWorksheet.String() + "'"
			return params
		},
		Prefix: "WOR",
		CreateInputSchema: func(scope string) (schema *jsonschema.Schema) {
			...
			var err error
			if schema, err = jsonschema.For[worksheetCreate](nil); err == nil {
				...
				schema.Required = []string{
					"trans_date", "direction", "customer_code", "currency_code", "due_time", "paid_type", "worksheet",
				}
			}
			return schema
		},
		UpdateInputSchema: func(scope string) (schema *jsonschema.Schema) {
			schema = &jsonschema.Schema{}
			var err error
			if schema, err = jsonschema.For[worksheetUpdate](nil); err == nil {
				schema.Properties["trans_date"].Type = "string"
				schema.Properties["trans_date"].Format = "date"
				schema.Properties["trans_map"].Default = []byte(`{}`)
				schema.Properties["due_time"].Type = "string"
				...
				schema.Properties["trans_state"].Type = "string"
				schema.Properties["trans_state"].Enum = ut.ToAnyArray(md.TransState(0).Keys())
				schema.Required = []string{"code", "worksheet"}
			}
			return schema
		},
		QueryInputSchema: func(scope string) (schema *jsonschema.Schema) {
			schema = &jsonschema.Schema{}
			schema, _ = jsonschema.For[worksheetParameter](nil)
			return schema
		},
		QueryOutputSchema: func(scope string) (schema *jsonschema.Schema) {
			schema = &jsonschema.Schema{}
			var err error
			if schema, err = jsonschema.For[worksheetQuery](nil); err == nil {
				schema.Description = "Worksheet data"
		...
		LoadData: worksheetLoadData,
		LoadList: func(rows []cu.IM) (items any, err error) {
			var worksheets []worksheetQuery = []worksheetQuery{}
			err = cu.ConvertToType(rows, &worksheets)
			return worksheets, err
		},
		Validate:      worksheetValidate,
...
func worksheetLoadData(data any) (modelData, metaData any, err error) {
	var worksheet md.Trans = md.Trans{
		TransType: md.TransTypeWorksheet,
	...
	err = cu.ConvertToType(data, &worksheet)
	return worksheet, worksheet.TransMeta, err
}

func worksheetCreateHandler(ctx context.Context, req *mcp.CallToolRequest, inputData worksheetCreate) (result *mcp.CallToolResult, response UpdateResponseData, err error) {
	...
	values := cu.IM{
		"trans_type": md.TransTypeWorksheet.String(),
	...
	cu.ConvertToType(inputData.worksheetMeta, &transMeta)
...
func worksheetValidate(ctx context.Context, input cu.IM) (data cu.IM, err error) {
	ds := ctx.Value(md.DataStoreCtxKey).(*api.DataStore)
	...
	if cu.ToBoolean(transMeta["closed"], false) {
		return data, errors.New("worksheet is not updatable because it is closed")
	}
	return input, nil
}
