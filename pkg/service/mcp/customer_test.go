...
func Test_customerCreateHandler(t *testing.T) {
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, _, gotErr := customerCreateHandler(ctx, tt.req, tt.inputData)
			if gotErr != nil {
				if !tt.wantErr {
					t.Errorf("customerCreateHandler() failed: %v", gotErr)
				}
				return
			}
			if tt.wantErr {
				t.Fatal("customerCreateHandler() succeeded unexpectedly")
			}
		})
	}
}
