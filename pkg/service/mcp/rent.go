...
func init() {
	toolDataMap["nervatura_rent_create"] = McpTool{
		Tool: mcp.Tool{
			Name:        "nervatura_rent_create",
			Title:       "Create a new rent",
			Description: "Create a new rent. Related tools: item.",
			Meta: mcp.Meta{
				"scopes": []string{"rent"},
			},
		},
		ModelSchema: RentSchema(),
		ConnectHandler: func(server *mcp.Server, tool *mcp.Tool) {
			mcp.AddTool(server, tool, rentCreateHandler)
		},
	}
	toolDataMap["nervatura_rent_query"] = McpTool{
		Tool: mcp.Tool{
			Name:        "nervatura_rent_query",
			Title:       "Query rents by parameters",
			Description: "Query rents by parameters. The result is all rents that match the filter criteria.",
			Meta: mcp.Meta{
				"scopes": []string{"rent"},
			},
		},
	...
	toolDataMap["nervatura_rent_update"] = McpTool{
		Tool: mcp.Tool{
			Name:        "nervatura_rent_update",
			Title:       "Update a rent by code",
			Description: "Update a rent by code. When modifying, only the specified values change. Related tools: item.",
			Meta: mcp.Meta{
				"scopes": []string{"rent"},
			},
		},
	...
	toolDataMap["nervatura_rent_delete"] = McpTool{
		Tool:        createDeleteTool("nervatura_rent_delete", "rent", mcp.Meta{"scopes": []string{"rent"}}),
		ModelSchema: RentSchema(),
		ConnectHandler: func(server *mcp.Server, tool *mcp.Tool) {
...
type rentCreate struct {
	TransDate    string `json:"trans_date" jsonschema:"Start date. Required when creating a new rent. Example: 2025-01-01"`
	Direction    string `json:"direction" jsonschema:"Transaction direction. Enum values. Required when creating a new rent. Example: DIRECTION_OUT"`
	...
	CustomerCode string `json:"customer_code" jsonschema:"Customer reference. Required when creating a new rent. Example: CUS1731101982N123"`
	...
	CurrencyCode string `json:"currency_code" jsonschema:"Currency iso code. Required when creating a new rent. Example: USD"`
	rentMeta
...
type rentUpdate struct {
	Code         string `json:"code" jsonschema:"Database independent unique key. Required when updating an existing rent."`
...
type rentMeta struct {
	DueTime       string           `json:"due_time,omitempty" jsonschema:"End date. Required when creating a new rent. Example: 2025-01-01"`
	...
	Closed        bool             `json:"closed,omitempty" jsonschema:"Closed rent"`
	...
	Rent          md.TransMetaRent `json:"rent" jsonschema:"Rent metadata. Required all fields."`
}
...
type rentQuery struct {
	...
	TransDate    string   `json:"trans_date,omitempty" jsonschema:"Rent date."`
	Direction    string   `json:"direction,omitempty" jsonschema:"Transaction direction. Enum values. Example: DIRECTION_OUT"`
	TransCode    string   `json:"trans_code,omitempty" jsonschema:"Other transaction (invoice, receipt, offer, rent, rent, rent etc.) reference. Optional. Example: ORD1731101982N123"`
	CustomerCode string   `json:"customer_code,omitempty" jsonschema:"Customer reference. Example: CUS1731101982N123"`
	EmployeeCode string   `json:"employee_code,omitempty" jsonschema:"Employee reference. Example: EMP1731101982N123"`
	ProjectCode  string   `json:"project_code,omitempty" jsonschema:"Project reference. Example: PRJ1731101982N123"`
	CurrencyCode string   `json:"currency_code,omitempty" jsonschema:"Currency iso code. Example: USD"`
	Amount       float64  `json:"amount,omitempty" jsonschema:"Total amount."`
	TransMeta    rentMeta `json:"trans_meta,omitempty" jsonschema:"Trans metadata."`
	TransMap     cu.IM    `json:"trans_map,omitempty" jsonschema:"Flexible key-value map for additional metadata. The value is any json type."`
}

type rentParameter struct {
	Code         string `json:"code,omitempty" jsonschema:"Database independent unique key."`
...
func RentSchema() (ms *ModelSchema) {
	return &ModelSchema{
		Name:       "trans",
		...
		CustomParameters: func(params cu.IM) cu.IM {
			params["filter"] = "trans_type = '" + md.TransTypeRent.String() + "'"
			return params
		},
		Prefix: "REN",
		CreateInputSchema: func(scope string) (schema *jsonschema.Schema) {
			...
			var err error
			if schema, err = jsonschema.For[rentCreate](nil); err == nil {
				...
				schema.Required = []string{
					"trans_date", "direction", "customer_code", "currency_code", "due_time", "paid_type", "rent",
				}
			}
			return schema
		},
		UpdateInputSchema: func(scope string) (schema *jsonschema.Schema) {
			schema = &jsonschema.Schema{}
			var err error
			if schema, err = jsonschema.For[rentUpdate](nil); err == nil {
				schema.Properties["trans_date"].Type = "string"
				schema.Properties["trans_date"].Format = "date"
				schema.Properties["trans_map"].Default = []byte(`{}`)
				schema.Properties["due_time"].Type = "string"
				...
				schema.Properties["trans_state"].Type = "string"
				schema.Properties["trans_state"].Enum = ut.ToAnyArray(md.TransState(0).Keys())
				schema.Required = []string{"code", "rent"}
			}
			return schema
		},
		QueryInputSchema: func(scope string) (schema *jsonschema.Schema) {
			schema = &jsonschema.Schema{}
			schema, _ = jsonschema.For[rentParameter](nil)
			return schema
		},
		QueryOutputSchema: func(scope string) (schema *jsonschema.Schema) {
			schema = &jsonschema.Schema{}
			var err error
			if schema, err = jsonschema.For[rentQuery](nil); err == nil {
				schema.Description = "Rent data"
		...
		LoadData: rentLoadData,
		LoadList: func(rows []cu.IM) (items any, err error) {
			var rents []rentQuery = []rentQuery{}
			err = cu.ConvertToType(rows, &rents)
			return rents, err
		},
		Validate:      rentValidate,
...
func rentLoadData(data any) (modelData, metaData any, err error) {
	var rent md.Trans = md.Trans{
		TransType: md.TransTypeRent,
	...
	err = cu.ConvertToType(data, &rent)
	return rent, rent.TransMeta, err
}

func rentCreateHandler(ctx context.Context, req *mcp.CallToolRequest, inputData rentCreate) (result *mcp.CallToolResult, response UpdateResponseData, err error) {
	...
	values := cu.IM{
		"trans_type": md.TransTypeRent.String(),
	...
	cu.ConvertToType(inputData.rentMeta, &transMeta)
...
func rentValidate(ctx context.Context, input cu.IM) (data cu.IM, err error) {
	ds := ctx.Value(md.DataStoreCtxKey).(*api.DataStore)
	...
	if cu.ToBoolean(transMeta["closed"], false) {
		return data, errors.New("rent is not updatable because it is closed")
	}
	return input, nil
}
