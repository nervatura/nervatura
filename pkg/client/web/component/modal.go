...
func modalShortcut(labels cu.SM, data cu.IM) (form ct.Form) {
	...
	fieldTypeMap := func(field cu.IM) ct.Field {
	...
	paramRows := func() (rows []ct.Row) {
		...
		for _, field := range fields {
			rows = append(rows, ct.Row{
				Columns: []ct.RowColumn{
					{Label: cu.ToString(field["description"], ""),
						Value: fieldTypeMap(field)}},
				Full: true,
			})
		}
		return rows
	...
	return ct.Form{
		Title:    cu.ToString(data["title"], labels["office_shortcut_title"]),
		Icon:     cu.ToString(data["icon"], ct.IconShare),
		Modal:    true,
		BodyRows: paramRows(),
		FooterRows: []ct.Row{
			{
