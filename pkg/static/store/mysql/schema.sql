...
CREATE VIEW trans_view AS
  SELECT id, code, trans_type, direction, trans_date, trans_code, customer_code, employee_code,
    ...
    CASE WHEN json_type(trans_meta->"$.worksheet") = 'OBJECT' THEN JSON_UNQUOTE(JSON_EXTRACT(JSON_EXTRACT(trans_meta,"$.worksheet"),"$.justification")) ELSE '' END AS worksheet_justification,
    ...
    CASE WHEN json_type(trans_meta->"$.rent") = 'OBJECT' THEN JSON_UNQUOTE(JSON_EXTRACT(JSON_EXTRACT(trans_meta,"$.rent"),"$.justification")) ELSE '' END AS rent_justification,
