...
const NT_LANG = "en"
const NT_HASHTABLE = "ref17890714"
const NT_TOKEN_ISS = "nervatura"
const NT_TOKEN_EXP = "1h"
const NT_TOKEN_KID = "8BD0E02509482F58CEFD3C0ECD41FF0E"
...
var lang = require('./lang.js')[process.env.NT_LANG || NT_LANG];
...
module.exports = function (nstore) {
  ...
  function check_hashtable(_callback) {
    var conn
    var hashtable = process.env.NT_HASHTABLE || NT_HASHTABLE
    async.waterfall([
      function (callback) {
        conn = nstore.connect.getConnect();
        if (!conn) {
        ...
        if(refname === ""){
          ...
          if(params.username){
            ...
            _sql = { 
              ...
              where: [["deleted", "=", "0"],["and","custnumber","=","'"+params.custnumber+"'"]]
            }
            refname = "customer"
          }
    ...
    async.waterfall([
      ...
      function (callback) {
        if (!nstore.employee() && nstore.customer()) {
          var _sql = {
            select: ["e.id", "e.username", "e.empnumber", "e.department", "ug.id as usergroup", "ug.groupvalue"], 
            from: ["employee e,", "groups ug"],
            where: [["e.inactive", "=", "0"], ["and", "e.deleted", "=", "0"],
            ["and", "e.username", "=", "'admin'"], ["and", "ug.groupname", "=", "'usergroup'"],
            ["and", "ug.groupvalue", "=", "'guest'"]]
          }
          conn.query(models.getSql(nstore.engine(), _sql), [], function (err, results) {
            if (!err) {
              if (results.rowCount > 0) {
  ...
  function auth_token(ustore, params) {
    ...
    if (ustore.employee()) {
      var token = { 
        iss: process.env.NT_TOKEN_ISS || NT_TOKEN_ISS, 
      ...
      return {
        token: jwt.sign(token,
          ...
          { 
            expiresIn: params.exp || process.env.NT_TOKEN_EXP || NT_TOKEN_EXP, 
            header: { kid: process.env.NT_TOKEN_KID || NT_TOKEN_KID }
          }
        )
      };
    } else {
