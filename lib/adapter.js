...
return {
  getConfig: function(engine, connect, settings, data_dir, app_config){
    var config = {adapter: engine}
    for (var key in connect) {
      var kvalue = connect[key].toString().trim();
      if (kvalue.length > 0) {
        if(kvalue.indexOf("$")>-1){
          var ekey = kvalue.substr(kvalue.indexOf("$")+1);
          var evalue = app_config.checkValue(ekey, null)
          if(evalue){
            if((ekey === "VCAP_SERVICES") && ((engine === "postgres") || (engine === "mysql"))){
              var vcap = JSON.parse(evalue);
              var ckey = (engine === "postgres") ? "compose-for-postgresql" : "compose-for-mysql";
              if(vcap[ckey]){
                vcap[ckey].forEach(element => {
                  if (connect.dbname === "" || connect.dbname === element.name){
                    connect.uri = element.credentials.uri; }});}}
            else if((engine === "mysql") && (ekey === "MYSQLCONNSTR_localdb")){
              var db_conn = String(evalue).split(";");
              db_conn.forEach(function(prop) {
                //console.log(prop);
                if(prop.split("=").length>1){
                  switch (prop.split("=")[0]) {
                    case "Database":
                      connect.dbname =prop.split("=")[1];
                      break;
                    case "Data Source":
                      connect.host =prop.split("=")[1].split(":")[0];
                      connect.port =prop.split("=")[1].split(":")[1];
                      break;
                    case "User Id":
                      connect.username =prop.split("=")[1];
                      break;
                    case "Password":
                      connect.password =prop.split("=")[1];
                      break;
                    default:
                      break;}}});}
            else {
              connect[key] = kvalue.replace("$"+ekey, app_config.checkValue(ekey, ekey));}}}}}
    switch (engine) {
        case "sqlite":
          config.adapter = "sqlite3";
          if(connect.dbname === "memory"){
        ...
        case "mssql":
          ...
          if (connect.port!==0 && connect.port!==null && connect.port!==""){
            config.port = connect.port;}
          config.options = {encrypt: app_config.checkValue(settings.db_ssl, settings.db_ssl || false)};
          config.database = connect.dbname;
          if (connect.username!==null && connect.username !=="") {
            config.user = connect.username;
            config.password = connect.password;}
      ...
      //console.log(config);
      if(config.uri){
        return {adapter: engine, uri:config.uri}}
      else {
        return config; }},
