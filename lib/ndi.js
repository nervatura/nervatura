...
function update_data(nstore, params, items, _callback){
  ...
  async.waterfall([    
    function(callback) {
      if (params.check_audit && !params.validator.valid){
                ...
                if (!item.hasOwnProperty("inputvalue") && !value_error){
                  ...
                  switch (item.inputmode) {
                    case "fxprice":
                      ...
                      item.vatamount = out.Round(item.fxprice*(1-item.discount/100)*item.rate_value*item.qty, digit);
                ...
                if (item.id === ""){
                  if (!item.hasOwnProperty("custnumber") && 
                    ...
                    if (!item.hasOwnProperty("cruser_id")) {
                      ...
                      else if (!params.check_audit && data.checklist.refvalue.employee["admin"]){
                        item.cruser_id = data.checklist.refvalue.employee["admin"].id; }}
      ...
      if (data){
        if (data.trans){
          if(data.trans.commit){
            data.trans.commit(function(cerr){
              _callback(cerr||null, result);});}
    ...
    function(data, callback) {
      ...
      switch (params.datatype) {
        ...
        case "item":
          ...
          if (data.data_audit !== "all" && nstore.employee()){
...
function get_api(nstore, params, _callback){
  ...
  if(params === null){
    return _callback({type:"error", id:"invalid", ekey:"message", err_msg: lang.unknown_method}, params); }
  var ndi_data = decode_data(params);
  if (ndi_data.error){
    return _callback({type: "error", id: "login", ekey: "message", err_msg: ndi_data.error}, params); }
  else {
    nstore.connect.getLogin(
      { database:ndi_data.params.database, username:ndi_data.params.username, 
        password:ndi_data.params.password, token:ndi_data.params.token },
      function(err, validator){
        if (err){
          return _callback({type: "error", id: "login", ekey: "message", err_msg: err}, params); }
        else if (nstore.ndi_enabled() === false){
          return _callback({type: "error", id: "login", ekey: "message", err_msg: lang.ndi_disabled}, params); }
        else {
          ...
          ndi_data.method(nstore, ndi_data.params, ndi_data.items, function(err, result){
            ...
            if (err){
              return _callback({type: "error", id: ndi_data.id, ekey: "message", err_msg: err}, params); }
            else {
              switch (ndi_data.output) {
                case "html":
                  return _callback({type: "html", tempfile: "data.html", data: result}, params);
                
                case "xml":
                  return _callback({type: "xml", tempfile: "data.xml", data: result}, params);
                ...
                case "csv":
                  ...
                  json2csv({data: result.data, fields:fields}, function(err, csv) {
                    if (err){
                      ...
                      return _callback({type: "csv", data: csv, filename: "NDIData"}, params); }});
                  break;
                
                default:
                  //json
                  return _callback({type: "json", id: result.id, data: result.data}, params); }}});}
    });}
}
