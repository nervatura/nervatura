              ...
              printer.items.forEach(function(item) {
                ...
                for (var c = 0; c < item.qty; c++) {
                  copies_lst.push(function(callback_){
                    idx += 1;
                    exec(cmd.replace("@tmp_file",tfile).replace("@qty",item.qty), (err, stdout, stderr) => {
                      callback_(err,[item.id,idx]);});});}
                doc_lst.push(function(_callback){
                  fs.writeFile(tfile, item.data, (err) => {
                    if (err){
                      _callback(err);}
                    else {
...
function get_price_value(nstore, params, _callback) {
  ...
  if (typeof params.posdate === "undefined") {params.posdate = get_iso_date();}
  ...
  async.waterfall([
    ...
    function(results, callback) {
      if(!results.report){
        if(!params.reportkey && !params.report_id){
          ...
          conn.query(models.getSql(nstore.engine(),_sql), [], function (err, data) {
            if (err){callback(err);}
            else if (data.rows.length === 0 && !params.refnumber){
              callback(nstore.lang().not_exist);}
            else {callback(null,{report:data.rows[0]})}});}}
      else {
    ...
    function(results, callback) {
      var _sql = {
        select:["rf.fieldname as fieldname","ft.groupvalue as fieldtype","rf.dataset as dataset",
          "wt.groupvalue as wheretype","rf.sqlstr as sql"], 
        ...
        if (err){callback(err);}
          ...
          data.rows.forEach(function(field) {
            results.fields[field.fieldname] = {
              fieldtype:field.fieldtype,wheretype:field.wheretype,dataset:field.dataset,sql:field.sql}});
          callback(null,results);}});},
    ...
    function(results, callback) {
      if(params.output === "tmp"){
        ...
        var _args; var fname;
        switch (results.report.reptype) {
          case "xls":
            fname = "create_excel";
            _args = {template:JSON.parse(results.report.report), data:results.datarows}
            break;
          case "ntr":
            fname = "create_report";
            _args = {
              orientation:params.orientation, size:params.size, output:params.output,
              template:results.report.report, data:results.datarows}
            break;}
        if (!_args){
          ...
          var ps = new PyShell("pylib.py", {
            args: [fname, JSON.stringify(_args)],
            pythonPath: nstore.server_config().python2_path,
            scriptPath: nstore.server_config().python_script,
          ...
          ps.end(function (err) {
            if (err) {callback(err);}
              ...
              if(params.output === "pdf" || results.report.reptype === "xls"){
                output = new Buffer(output, 'base64');}
              callback(null,{filetype:results.report.reptype, 
                template:output,data:null});}});}}}
  ],
...
function get_iso_datetime(cdate,full,nosep) {
  ...
  if (full) {
    if (nosep){
      ...
      return get_iso_date(cdate)+" "+zero_pad(cdate.getHours(),2)+":"+
        zero_pad(cdate.getMinutes(),2)+":00";}}
  else {
    return get_iso_date(cdate)+" 00:00:00";}}
