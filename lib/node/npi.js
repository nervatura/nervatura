...
function get_login(nstore, params, _callback) {
  async.waterfall([
    function(callback) {
      nstore.connect.getLogin({database:params.database, username:params.username, password:params.password},
    ...
    function(validator, callback) {
      ...
      validator.employee = nstore.employee();
    ...
    function(validator, callback) {
      var _sql = {select:["value"], from:"fieldvalue",
        where:[["ref_id","is","null"],["and","fieldname","=","'log_userlogin'"]]}
      validator.conn.query(models.getSql(nstore.engine(), _sql), [], 
        function (err, results) {
          if (err) {
...
function set_data(nstore, login, params, _callback) {
  async.waterfall([
    function(callback) {
      var validator = login;
      if (!validator.conn){
        nstore.connect.getLogin({database:login.database, username:login.username, password:login.password},
          function(err, data){
  ...
  function(err, validator, results) {
    ...
    _callback(err, results, params.infoName);});};
  ...
  async.waterfall([    
    function(callback) {
      nstore.connect.getLogin({database:params.login.database, username:params.login.username, 
        password:params.login.password}, function(err, validator){
          if (err){
            callback(err, validator);}
