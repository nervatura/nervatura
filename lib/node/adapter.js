...
return {
  getConfig: function(engine, connect, data_dir, app_config){
    ...
    for (var key in connect) {
      ...
      if (kvalue.length > 0) {
        if(kvalue.indexOf("$")>-1){
          ...
          if(process.env[ekey]){
            if((engine === "mysql") && (ekey === "MYSQLCONNSTR_localdb")){
              var db_conn = String(process.env[ekey]).split(";");
              ...
              connect[key] = kvalue.replace("$"+ekey, process.env[ekey]);}}}}}
    switch (engine) {
        case "sqlite":
        ...
        case "mysql":
          ...
          config.ssl = app_config.mysql_ssl;
        ...
        case "mssql":
          ...
          if (connect.port!==0 && connect.port!==null && connect.port!==""){
            config.port = connect.port;}
          config.options = {encrypt: app_config.mssql_encrypt};
          config.database = connect.dbname;
          if (connect.username!==null && connect.username !=="") {
            config.user = connect.username;
            config.password = connect.password;}
