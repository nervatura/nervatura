...
function valid(req){
  if (req.app.settings.conf.host_restriction.nas.length>0 && req.app.settings.conf.host_restriction.nas.indexOf(req.ip)===-1){
    return "insecure";}
  else if (req.app.settings.conf.host_restriction.nas.length===0 && req.app.settings.conf.host_restriction.all.length>0 
...
function restore_list(params){
  fs.readdir(params.data.backup_dir, function(err, files){
    if (err) {return params.next(err);}
    else {
      params.data.filenames = files; params.data.filenames.unshift("");
...
function restore_database(nstore, params, _callback){
  var conn; var logstr = "";
  if(params.logstr){logstr = params.logstr;}
  async.waterfall([
    function(callback) {
      ...
      logstr += '<span>'+nstore.lang().label_backup_file+': '
        +params.filename+'</span><br>';
      logstr += '<div><span>'+nstore.lang().log_start_process+': '
        +out.getISODateTime(new Date(),true)+'</span></div>';
      if (!params.filename || params.filename===""){
        callback(nstore.lang().missing_required_field+" "+nstore.lang().label_backup_file);}
    ...
    function(callback) {
      ...
      if (!conn){
        ...
        logstr += '<div><span>'+nstore.lang().log_load_backup+'</span></div>';
        fs.readFile(path.join(params.backup_dir, params.filename),function(err, data) {
          callback(err, data);});}},
    
    function(data, callback) {
      if(params.filename.indexOf(".backup") > -1){
    ...
    function(data, callback) {
      ...
      models.backupList.nom.forEach(function(nom) {
        if(data.data[nom]){
          var items = [];
    ...
    function(data, callback) {
      var value_lst = [];
      models.backupList.ui_1.forEach(function(ui) {
        var ui_count = 0;
        if(data.data[ui]){
    ...
    function(data, callback) {
      var value_lst = [];
      models.backupList.ui_2.forEach(function(ui) {
        var ui_count = 0;
        if(data.data[ui]){
          data.data[ui].forEach(function(fields) {
            var _sql = {}; var prm = []; ui_count +=1;
  ...
  function(err) {
    if(err){if(err.message){err = err.message;}}
    if (!err){
      logstr += '<div><span style="font-weight: bold;">'+nstore.lang().info_restore_ok+'</span></div>';}
    else {
      logstr += '<div><span style="color:red;font-weight: bold;">'+nstore.lang().log_error+': '+err+'</span></div>';}
    if (conn){conn.close();}
    logstr += '<div><span>'+nstore.lang().log_end_process+': '+out.getISODateTime(new Date(),true)+'</span></div>';
...
function backup_database(nstore, params, _callback){
  ...
  async.waterfall([
    ...
    function(nom_data, callback) {
      ...
      var ui_lst = models.backupList.ui_1.concat(models.backupList.ui_2);
    ...
    function(result, callback) {
      var xml_params = {dbs:result, version:params.version,
        timestamp:timestamp}
      nstore.res().render("nas/backup.xml", xml_params, function(err,xml){
        callback(err, xml);});},
    
    function(result, callback) {
      if (params.format === "backup"){
        params.bck_filename = params.database+"_"+timestamp+".backup"
    ...
    function(result, callback) {
      if(params.filename !== "download") {
        fs.writeFile(path.join(params.backup_dir,params.bck_filename), result, function (err) {
          if(!err){
            logstr += '<br><div><span>'+nstore.lang().label_backup+': </span>';
            logstr += '<a href="/backup/'+params.bck_filename
              +'" target="_blank" data-ajax="false" >'+params.bck_filename+'</a></div>';}
          callback(err, result);});}
  ...
  function(err, result) {
    ...
    if(err || params.filename !== "download"){
      if (!err){
        logstr += '<div><span style="font-weight: bold;">'
          +nstore.lang().info_backup_ok+'</span></div>';}
      else {
...
return {
  ...
  backupDatabase: function(nstore, params, _callback) {
    backup_database(nstore, params, _callback);},
  restoreDatabase: function(nstore, params, _callback) {
    restore_database(nstore, params, _callback);},
  
  reportRender: function(params) {
    report_render(params);},
  reportList(nstore, form, flash) {
    report_list(nstore, form, flash);},
  reportInstall(nstore, form) {
    report_install(nstore, form);},
  reportUpdate(nstore, params, _callback) {
    report_update(nstore, params, _callback);},
  reportDelete(nstore, params, _callback) {
    report_delete(nstore, params, _callback);},
  reportEdit(nstore, form) {
