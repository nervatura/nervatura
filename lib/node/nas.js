...
function valid(req){
  if (req.app.get("host_settings").nas_host_restriction.length>0 && req.app.get("host_settings").nas_host_restriction.indexOf(req.ip)===-1){
    ...
    return "ok";}
  else {return "/nas/login";}}

function valid_setting(params, lang){
    ...
    case "nas_auth":
      switch (params.value) {
        ...
        case "google":
          return(null);  
        default:
          return(lang.invalid_fieldname+" "+params.value+" (local, google)");}
      break;
...
return {
  validUser: function(req) {
    return valid(req);},
  validSetting: function(params, lang) {
    return valid_setting(params, lang);},
