...
function dataStore(options) {
    ...
    if (!err) {
      if (entities.length === 0){
        ...
        for (index = 0; index < databases.length; index++) {
          items.push({
            key: store.key(['dbs', databases[index].alias]),
            data: {
              id: new Date().getTime(),
              alias: databases[index].alias, 
        ...
        store.save(items, function(err, apiResponse) {
          if(options.callback){options.callback(err, settings);}});}
      else {
  ...
  function update_data(collection, key, values, callback){
    ...
    entity = {key: store.key([collection, key]), data: values}
  ...
  function delete_data(collection, key, callback){
    ...
    store.get(store.key([collection, username]), function (err, data) {
      if (err) {
        return callback(err);}
      else if (!data) {
        if (collection === "user"){
  ...
  function change_password(username, old_passw, new_passw, ver_passw, callback){
    store.get(store.key(['user', username]), function (err, data) {
      ...
      if (data.password != get_md5_password(old_passw)) {
        ...
        entity = {key: store.key(["user", username]), data: data}
  ...
  return {
    ...
    getUserFromEmail: function(profile, token, callback){
      get_user({username: profile.emails[0].value, password: null, 
