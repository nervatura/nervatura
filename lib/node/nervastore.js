  ...
  async.waterfall([
            ...
            _sql = {
              ...
              from: [[[
                ...
                where:[["groupname","in",[[],"'nervatype'","'custtype'","'fieldtype'","'logstate'","'movetype'",
                  "'transtype'","'placetype'","'storetype'","'calcmode'","'protype'","'ratetype'","'direction'",
    ...
    function(ref_id, logical_delete, callback) {
      if (logical_delete === false) {
        ...
        var _sql = {delete:"", from:"fieldvalue", 
          where:["id","in",
...
function get_connect(self) {
  try {
    return connect.createConnection({connect:self.dbs_config, pool:true, config:self.pool_config});}
  catch (error) {
    return null;}}
...
function insert_log(self, params, _callback) {
  ...
  async.waterfall([
    function(callback) {
      if (self.employee !== null && params.nervatype !== "log" && (typeof params.logstate !== "undefined")) {
        if ((typeof params.nervatype === "undefined") || (params.nervatype === null)) {
          params.nervatype = "";}
        var _sql = [{
      ...
      conn.query(models.getSql(self.engine, _sql), [params.username], function (error, results) {
        if (error) {err_msg = error;}
          ...
          if (results.rowCount>0) {
            ...
            if (params.password !== results.rows[0].password) {
              err_msg = self.lang.wrong_password;}
            else {
              self.employee = results.rows[0];
              validator.valid = true;
              validator.conn = conn;}}
          else {
            err_msg = self.lang.unknown_user;}}
          if (err_msg){
            //conn.close(); 
            callback(err_msg);}
          else {callback(null);}});}
...
function set_engine(self, params, _callback) {
  ...
  async.waterfall([    
    ...
    function(database, callback) {
      if (database.settings.ndi_host_restriction!==null && database.settings.ndi_host_restriction!==""){
        if (database.settings.ndi_host_restriction.split(",").indexOf(self.req.ip)===-1) {
          callback("NDI "+self.lang.insecure_err);}
        else {callback(null,database);}}
      else {callback(null,database);}},
    
    function(database, callback) {
      if (params.check_ndi) {
        if (database.settings.ndi_enabled === false) {
          callback(self.lang.ndi_disabled);}
        else {
          self.md5_password = database.settings.ndi_md5_password;
          self.encrypt_data = database.settings.ndi_encrypt_data;
          self.encrypt_password = database.settings.ndi_encrypt_password;
          callback(null,database);}}
      else{callback(null,database);}},
      
    function(database, callback) {
      var config = connect.getConfig(database.engine, database.connect);
...
module.exports = function(_req, _res) {
  ...
  self.req = _req; 
  self.res = _res;
  self.storage = _req.app.locals.settings.storage;
  self.report_dir = _req.app.locals.settings.report_dir;
  self.lang = _req.app.locals.lang;
  self.md5_password = false;
  self.encrypt_data = false;
  self.encrypt_password = null;
  self.server_config = _req.app.locals.settings.conf;
  self.pool_config = _req.app.locals.settings.conf.pool_config;
