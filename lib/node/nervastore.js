...
function get_connect(self) {
  try {
    return connect.createConnection({connect:self.dbs_config, pool:true, config:self.server_config.pool_config});}
  catch (error) {
    return null;}}
  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      conn.query(models.getSql(self.engine, _sql), [params.username], function (error, results) {
        if (error) {err_msg = self.lang.missing_database+params.database;}
        else {
...
function set_engine(self, params, _callback) {
  ...
  async.waterfall([    
    ...
    function(database, callback) {
      self.ndi_enabled = database.settings.ndi_enabled;
      self.encrypt_password = database.settings.encrypt_password;
      if (database.settings.dbs_host_restriction!==null && database.settings.dbs_host_restriction!==""){
        if (database.settings.dbs_host_restriction.split(",").indexOf(self.req.ip)===-1) {
          callback("Database "+self.lang.insecure_err);}
        else {callback(null,database);}}
      else {callback(null,database);}},
      
    function(database, callback) {
      var config = connect.getConfig(database.engine, database.connect, self.req.app.get("data_dir"));
...
module.exports = function(_req, _res) {
  ...
  self.req = _req; 
  self.res = _res;
  self.storage = _req.app.get("storage");
  self.report_dir = _req.app.get("report_dir");
  self.lang = _req.app.locals.lang;
  self.ndi_enabled = true;
  self.encrypt_password = null;
  self.server_config = _req.app.get("conf");
