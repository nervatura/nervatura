...
function get_connect(self) {
  try {
    return connect.createConnection({connect:self.dbs_config, pool:true, config:self.app_config.pool_config});}
  catch (error) {
    return null;}}
...
module.exports = function(_req, _res) {
  ...
  self.app_config = _req.app.get("conf");
  self.host_settings = _req.app.get("host_settings");
