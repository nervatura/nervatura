...
function get_refnumber(self, params, _callback){
  ...
  if (!params.conn){
    ...
    if (!conn){return _callback(lang.not_connect);}}
  
  async.waterfall([
    function(callback) {
      if (!params.nervatype || !params.ref_id){
        callback(lang.ndi_missing_refnumber,null);}
      else {
        var _sql;
    ...
    function(callback) {
      ...
      conn.query(models.getSql(self.engine, _sql), [], function (error, data) {
        if (error) {
          ...
          if (data.rowCount===0) {
            callback(lang.missing_fieldname, params.value);}
          else {
      ...
      switch (fieldtype) {
        ...
        case "integer":
          if (isNaN(parseInt(params.value,10))) {
            callback(lang.invalid_value+": "+fieldtype, params.value);}
          else {
...
function get_id_from_refnumber(self, params, _callback){
      ...
      switch (nervatype) {
        ...
        case "contact":
          ...
          if (refnumber.split("/").length > 1) {
            ...
            else {err = lang.invalid_refnumber;}}
          else {
            err = lang.invalid_refnumber;}
          break;
        ...
        case "price":
          ...
          //partnumber~pricetype~validfrom~curr~qty
          if ((refnumber.split("~").length === 5) || refnumber.split("~").length === 4) {
            var pricetype = "price"
            var validfrom = refnumber.split("~")[1]; var curr = refnumber.split("~")[2]; 
            var qty = parseFloat(refnumber.split("~")[3]);
            if(refnumber.split("~").length === 5){
              pricetype = refnumber.split("~")[1];
              validfrom = refnumber.split("~")[2];
              curr = refnumber.split("~")[3]; 
              qty = parseFloat(refnumber.split("~")[4]);
            } 
            if(isNaN(qty)){err = lang.invalid_refnumber;}
            if (pricetype === "price") {
        ...
        case "ui_audit":
          ...
          if (refnumber.split("~").length > 1) {
            vsql = {select:["au.id as id"], from:"ui_audit au",
              ...
              where:[["nt.groupvalue","=","'"+refnumber.split("~")[1]+"'"]]};
  ...
  if (!conn){
    conn = get_connect(self);
    if (!conn){return _callback(lang.not_connect, null, info);}}
  async.waterfall([
    function(callback) {
      if(params.nervatype === null || params.refnumber === null) {
        callback(lang.invalid_refnumber);}
      else {
        callback(null);}},
      ...
      if (params.nervatype === "fieldvalue"){
        conn.query(vsql, [refnumber], function (error, data) {
          if (error) {callback(error, null);}
            ...
            if (data.rowCount === 0) {
              callback(lang.invalid_refnumber.replace("refnumber",params.nervatype)+params.refnumber, null);}
            else {
...
function get_data_audit(self, params, _callback){
  ...
  if (self.employee === null){
    _callback(lang.ndi_invalid_login);}
  else if (self.employee.usergroup === null){
    _callback(lang.missing_usergroup);}
  else {
    var conn;
    if(params.conn){
      conn = params.conn;}
...
function get_object_audit(self, params, _callback){
  ...
  if (self.employee === null){
    ...
    _callback(lang.missing_nervatype);}
  ...
  async.waterfall([
    function(callback) {
      ...
      if (params.nervatype === null) {
        callback(lang.missing_nervatype);}
        ...
        get_id_from_refnumber(self, {nervatype:params.nervatype, refnumber:params.refnumber}, 
          function(err, id, info){
            if (err) {
              ...
              callback(lang.missing_fieldname+": "+params.refnumber);}
            else {callback(null, id);}})}
      else {
        callback(lang.missing_fieldname+": ref_id or refnumber");}},
    ...
    function(ref_id, callback) {
      ...
      if (["address","barcode","contact","event","fieldvalue","item","link","log","movement","pattern",
        ...
        callback(lang.integrity_error, ref_id);}
      else {
        var _sql;
        switch (params.nervatype) {
          case "currency":
        ...
        if(_sql){
          ...
          if (params.nervatype.substr(0,3) === "ui_" ){
            callback(null, ref_id);}
          else {callback(lang.integrity_error, ref_id);}}}},
...
function get_connect(self) {
  try {
    return connect.createConnection(
      {
        connect: self.dbs_config, pool: true,
        config: { 
          min: process.env.NT_POOL_MIN || 0, 
          max: process.env.NT_POOL_MAX || 10 
        }
      });
  }
  catch (error) {
  ...
  async.waterfall([
    function(callback) {
      if (params.numberkey === null){
        callback(lang.missing_required_field+": numberkey");}
      else {
    ...
    function(data, callback) {
      if (data.rowCount === 0) {
        if (params.insert_key) {
          ...
          callback(lang.invalid_value+": "+params.numberkey);}}
...
function get_login(self, params, _callback) {
  ...
  async.waterfall([
    ...
    function(conn, callback) {
      ...
      conn.query(models.getSql(self.engine, _sql), [params.username], function (error, results) {
        if (error) {
          callback(lang.missing_database+params.database);}
        else {
          validator.conn = conn;
          if (results.rowCount>0) {
  ...
  async.waterfall([
    function(callback) {
      if (typeof ntura.model[nervatype] === "undefined") {
        callback(lang.invalid_nervatype+" "+nervatype);}
      else {
          ...
          default:
            if (typeof ntura.model[nervatype][fieldname] !== "undefined") {
              ...
              if (valid_fieldvalue(nervatype)){
                ...
                fieldname_err = lang.unknown_fieldname+": "+fieldname;}}
            break;}}
            ...
            for (var fieldname in check_values.values) {
              if (!req_error){
                ...
                switch (field.type) {
                  case "integer":
                    if (!field.hasOwnProperty("requires")){
                      if (value === null){
                        ...
                        req_error = lang.invalid_value+": "+value+" (integer)";}
                      else {
                  ...
                  case "float":
                    if (value === null){
                      ...
                      req_error = lang.invalid_value+": "+value+" (float)";}
                    else {
                ...
                if ((check_values.values[fieldname] === null) && (field.notnull === true)) {
                  req_error = lang.missing_required_field+" "+fieldname;}
                  ...
                  if (field.requires.hasOwnProperty("bool")){
                    ...
                    if (value === "" || value === null){
                      ...
                      if (typeof curr[value] === "undefined") {
                        req_error = lang.invalid_value.replace("value",fieldname)+": "+value;}}}
                    ...
                    if (field.type === "integer" || field.type === "float"){
                      if (field.requires.hasOwnProperty("min") && check_values.values[fieldname]<field.requires.min){
                        req_error = lang.invalid_value+": "+check_values.values[fieldname]+" ("+fieldname+" min. value:"+field.requires.min+")";}
                      if (field.requires.hasOwnProperty("max") && check_values.values[fieldname]>field.requires.max){
                        req_error = lang.invalid_value+": "+check_values.values[fieldname]+" ("+fieldname+" max. value:"+field.requires.max+")";}}}
                  else {
                    if (value === "" || value === null){
                      check_values.values[fieldname] = null}
                    else if (!groups.hasOwnProperty(value)){
                      req_error = lang.invalid_value+": "+value;}
                    else {
                      var gvalid;
                      for (var req in field.requires) {
                        if (groups[value].groupname === req) {
                          ...
                          if (field.requires[req].indexOf(groups[value].groupvalue) > -1){
                            gvalid = true;}}}
                      if (!gvalid){
                        req_error = lang.invalid_value+": "+groups[value].groupvalue;}}}}}}
    ...
    function(ref_id, check_values, callback) {
      if ((Object.keys(check_values.values).length>0) && (nervatype === "fieldvalue")){
        ...
        if (typeof check_values.values.fieldname === "undefined") {
          callback(lang.missing_fieldname);}
        else {
        ...
        for (var fieldname in check_values.fvalues) {
          ...
          if (typeof check_values.dvalues[fieldname] === "undefined") {
            if (params.insert_field === false) {
              fieldname_err = lang.missing_insert_field+" "+fieldname;}
      ...
      self.storage.getDbsFromAlias(params.database, function(err, database){
        if(err !== null){
          if(err.message === "missing" || err === "missing"){
            err = lang.missing_database+" "+params.database;}}
    ...
    function(database, callback) {
      ...
      if (host_restriction!==""){
        if (host_restriction.split(",").indexOf(self.host_ip)===-1) {
          callback("Database "+lang.insecure_err);}
...
return {
  ...
  report_dir: function(){
    if(!self.report_dir){
      if(process.env.NT_REPORT_DIR){
        return process.env.NT_REPORT_DIR
      } else {
        return path.join(__dirname, path.join("..","report"))
      }
    }
  ...
  dbs_config: function (config) {
    if (config) { self.dbs_config = config; }
    return self.dbs_config;
  },
  app_config: function(){return self.app_config;},
  host_settings: function(){return self.host_settings;},
  employee: function (employee) {
    if (employee) { self.employee = employee; }
