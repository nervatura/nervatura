...
module.exports = function () {
  ...
  function get_login(nstore, params, _callback) {
    async.waterfall([
      function (callback) {
        if(params.api_login){
          api().AuthUserLogin(params, function(err, token, ustore){
            if(!err){
              nstore = ustore;
              callback(null, {
                valid: true,
                employee: nstore.employee(),
                token: token,
  ...
  function set_data(nstore, login, params, _callback) {
    async.waterfall([
      function (callback) {
        var validator = Object.assign({}, login);
        if (!validator.conn) {
          if(validator.token_key){
            ...
            nstore.connect.getLogin({
              database: login.database, username: login.username,
      ...
      function (validator, callback) {
        switch (params.method) {
          case "function":
            ...
            if (typeof service[params.functionName] !== "undefined") {
              params.paramList.conn = validator.conn;
              service[params.functionName](nstore, params.paramList, function (err, data) {
          ...
          case "update":
            var update_params = {
              nervatype: params.record.__tablename__, values: params.record,
          ...
          case "delete":
            var delete_params = {
              nervatype: params.record.__tablename__, ref_id: params.record.id,
              refnumber: params.record.refnumber, transaction: params.transaction, validator: validator
            };
          ...
          default:
            ...
            switch (params.method) {
              ...
              case "view":
                ...
                break;
              case "execute":
                sql = nstore.local.setSqlParams({
                  sqlStr: params.sqlStr, whereStr: "", havingStr: "",
  ...
  function load_dataset(nstore, params, _callback) {
    async.waterfall([
      function (callback) {
        if(params.login.token_key){
          var validator = Object.assign({}, params.login);
          validator.conn = nstore.connect.getConnect();
          if (!validator.conn) {
            callback(lang.not_connect);
          } else {
          ...
          nstore.connect.getLogin({
            database: params.login.database, username: params.login.username,
            password: params.login.password, token: params.login.token
          }, function (err, validator) {
            if (err) {
              callback(err, validator);
            }
        ...
        async.series(results_lst, function (err) {
          ...
          if (err) {
            ...
            if (trans.commit) {
              trans.commit(
                function (cerr) {
                  callback(cerr || null, validator, results);
                });
            }
            else {
              callback(err, validator, results);
      ...
      function (validator, callback) {
        ...
        params.dataSet.forEach(function (updateSetInfo) {
          switch (updateSetInfo.updateType) {
            ...
            case "function":
              var function_params = {
                method: "function", functionName: updateSetInfo.functionName,
