...
import '../NtDate/nt-date.js'

import { DATE_TYPE } from '../NtDate/NtDate.js'
import { styles } from './NtField.styles.js'

export class NtField extends LitElement {
  constructor() {
    super();
  ...
  render() {
    ...
    switch (datatype) {
      ...
      case "flip":
        ...
        if([1,"1","true",true].includes(value)){
          return html`<div id="${this.id}"
            ...
            <nt-icon iconKey="ToggleOn" width=40 height=32.6 ></nt-icon>
          </div>`
        }
        return html`<div id="${this.id}"
          name="${fieldName}" style="${styleMap(this.style)}" 
          ...
          <nt-icon iconKey="ToggleOff" width=40 height=32.6 ></nt-icon>
        </div>`
      ...
      case "select":
        ...
        return html`<nt-select id="${this.id}" ?full="${true}"
            name="${fieldName}" label="${fieldName}"
            .style="${this.style}"
            ?disabled="${disabled}" 
            .onChange=${(v) => {
            ...
            .isnull="${empty}" value="${value}" ></nt-select>`
      ...
      case "valuelist":
        return html`<nt-select id="${this.id}" ?full="${true}"
          ...
          .isnull="${false}" value="${value}" ></nt-select>`
      ...
      case "selector":
        ...
        if(!disabled){
          columns.push(html`<div id="sel_show" class="cell search-col">
            <nt-button id="${`sel_show_${fieldName}`}" label="${`sel_show_${fieldName}`}"
              .style="${{ padding: "5px 8px 7px", "border-radius": "3px" }}"
              @click=${()=>this._onSelector(selector.type, selector.filter, (...args) => this.setSelector(...args))} 
              type="border" ><nt-icon iconKey="Search" ></nt-icon></nt-button>
          </div>`)
        }
        if (empty) {
          columns.push(html`<div id="sel_delete" class="cell times-col">
            <nt-button id="${`sel_delete_${fieldName}`}" label="${`sel_delete_${fieldName}`}"
              .style="${{ padding: "5px 8px 7px", "border-radius": "3px" }}"
              ?disabled="${disabled}"
              @click=${(!disabled) ? ()=>this.setSelector() : null} 
              type="border" ><nt-icon iconKey="Times" ></nt-icon></nt-button>
          </div>`)
        }
      ...
      case "button":
        return html`<nt-button id="${this.id}" 
          name="${fieldName}" type="border"
          .style="${{ padding: "7px 8px", ...this.style }}"
          ?disabled="${disabled}" label="${fieldName}" ?full="${this.field.full}"
          ?autofocus="${this.field.focus || false}"
          @click=${(!disabled) ? ()=>this._onChange({ 
            value: fieldName, item: {}, event_type: "click", editName, fieldMap 
          }) : null} >
          <nt-label value="${(this.field.title)?(this.field.title):""}" 
            leftIcon="${this.field.icon}" ></nt-label>
        </nt-button>`
      ...
      case "float":
        ...
        return html`<nt-number id="${this.id}" name="${fieldName}"
      ...
      default:
        ...
        return html`<nt-input 
          id="${this.id}" .id="${`${this.id}_input`}" name="${fieldName}" type="text" 
          value="${value||""}" label="${fieldName}"
          ?full="${true}"
          .style="${this.style}" 
          maxlength="${(this.field.length) ? this.field.length : undefined}"
          size="${(this.field.length) ? this.field.length : undefined}"
          ?disabled="${disabled}"
          .onChange=${
            (event) => this._onChange({value: event.value, event_type: "change", editName, fieldMap}) 
          }></nt-input>`
