...
import { styles } from './NtDate.styles.js'
...
export class NtDate extends LitElement {
  constructor() {
    super();
    this.id = Math.random().toString(36).slice(2)
    this.name = undefined
    this.value = ""
    this.type = DATE_TYPE.DATE
  ...
  static get properties() {
    return {
      ...
      type: { 
        type: String, 
        converter: (value) => {
          if(!Object.values(DATE_TYPE).includes(value)){
  ...
  connectedCallback() {
    ...
    if((this.type === DATE_TYPE.DATE) && (this.value.length > 10)){
