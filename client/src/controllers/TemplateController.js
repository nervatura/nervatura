import { APP_MODULE, SIDE_EVENT, TEMPLATE_EVENT, SIDE_VISIBILITY, MODAL_EVENT, TOAST_TYPE } from '../config/enums.js'
...
export class TemplateController {
  ...
  checkTemplate(cbKeyTrue) {
    ...
    const nextKeys = {
      ...
      [SIDE_EVENT.LOAD_SETTING]: () => currentModule({ 
        data: { module: APP_MODULE.SETTING, side: SIDE_VISIBILITY.HIDE },
        content: { fkey: "checkSetting", args: [ { type: 'template' }, SIDE_EVENT.LOAD_SETTING ] } 
      })
    }
  ...
  createTemplate() {
    ...
    let reportkey = data[APP_MODULE.TEMPLATE].template.meta.nervatype;
    if (reportkey === "trans") {
      reportkey = `${data[APP_MODULE.TEMPLATE].template.meta.transtype}_${data[APP_MODULE.TEMPLATE].template.meta.direction}`;
    }
    const timeValue = new Date()
    reportkey += `_${new Date(timeValue).toISOString().slice(0,10)}${new Date(timeValue).toLocaleTimeString("en",{hour12: false}).replace("24","00")}`.replaceAll("-","").replaceAll(":","")
    const  modalForm = inputBox({ 
      title: msg("", { id: "template_label_new" }),
      message: reportkey,
      value: data[APP_MODULE.TEMPLATE].dbtemp.repname, showValue: true,
      onEvent: {
        onModalEvent: async (modalResult) => {
          setData("current", { modalForm: null })
          if ((modalResult.key === MODAL_EVENT.OK) && (modalResult.data.value !== "")) {
            const template = {...data[APP_MODULE.TEMPLATE].template,
              meta: {...data[APP_MODULE.TEMPLATE].template.meta,
                reportkey,
                repname: modalResult.data.value
              }
            }
            const { orientation, size, ...values} = {
              ...tableValues("report", data[APP_MODULE.TEMPLATE].dbtemp),
              id: null,
              reportkey,
              repname: modalResult.data.value,
              report: JSON.stringify(template)
            }
            const result = await requestData("/ui_report", { method: "POST", data: [values] })
            if(result.error){
              return resultError(result)
            }
            return currentModule({ 
              data: { module: APP_MODULE.SETTING, side: SIDE_VISIBILITY.HIDE },
              content: { fkey: "checkSetting", args: [ { type: 'template', id: result[0] }, SIDE_EVENT.LOAD_SETTING ] } 
            })
          }
          return true
        }
    ...
    const  modalForm = inputBox({ 
      ...
      onEvent: {
        onModalEvent: async (modalResult) => {
          setData("current", { modalForm: null })
          if (modalResult.key === MODAL_EVENT.OK) {
            const result = await requestData("/ui_report", 
              { method: "DELETE", query: { id: data[APP_MODULE.TEMPLATE].id } })
  ...
  showPreview(orient) {
    ...
    const loadPreview = async (params) => {
      const result = await requestData("/report", { method: "POST", data: params })
      if(result.error){
        resultError(result)
      } else {
    ...
    if(["_blank","_sample"].includes(setting.key)){
      ...
      const  modalForm = inputBox({ 
        ...
        message: msg("", { id: "template_preview_input" }).replace("docname",params.nervatype),
        ...
        onEvent: {
          onModalEvent: async (modalResult) => {
            ...
            if ((modalResult.key === MODAL_EVENT.OK) && (modalResult.data.value !== "")) {
              ...
              params.refnumber = modalResult.data.value
              loadPreview(params)
            }
            return true
          }
