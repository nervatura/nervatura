export const Sql = (app) => {
  ...
  return {    
    ...
    currency: {
      delete_state: () => {
        const _sql = {select:["sum(co) as sco"], from:[
      ...
      currency_view: () => {
        const _sql = {select:["*"], from:"currency"}; return _sql;}
      },
    ...
    event: {
      event: () =>{
        const sql = {select:["*","{FMS_DATETIME}fromdate {FME_DATETIME} as fromdate",
      ...
      result: () => {
        const sql = {
          select:["{FMS_DATE}l.crdate {FME_DATE} as crdate","e.empnumber","ls.groupvalue as logstate",
            "nt.groupvalue as nervatype","l.ref_id as refnumber","l.id as id"],
          from:"log l",
          inner_join:[
          ...
          if (params.nervatype !== "" && 
            params.nervatype !== null) {
              switch (params.nervatype) {
                ...
                case "tool":
                case "project":
                  sql.where.push(["and","ntg.groupvalue","=",`'${params.nervatype}'`]);
                  break;
                default:
                  sql.where.push(["and","ntg.groupvalue","=","'trans'"]);
                  sql.where.push(["and","tg.groupvalue","=",`'${params.nervatype}'`]);
                  break;}}
          if (params.startdate !== "" && 
            params.startdate !== null) {
            sql.where.push(["and","pq.crdate",">=",`'${params.startdate}'`]);}
          if (params.enddate !== "" && 
            params.enddate !== null) {
            sql.where.push(["and","pq.crdate","<=",`'${params.enddate}'`]);}
          if (params.transnumber !== "" && 
            params.transnumber !== null) {
            sql.where.push(["and", "case when ntg.groupvalue in ('customer') then rf_customer.custnumber "+
              ...
              "when ntg.groupvalue in ('employee') then rf_employee.empnumber "+
              "when ntg.groupvalue in ('place') then rf_place.planumber "+
              "else null end","like",`'${params.transnumber}'`]);}
          if (params.username !== "" && 
            params.username !== null) {
            sql.where.push(["and","e.username","like",`'${params.username}'`]);}}
        return sql;}
    },
    ...
    trans: {
      ...
      trans: () => {
        ...
        invoice_customer: () => {
          ...
          from: [
            [[{select:["c.custname","adr.zipcode","adr.city","adr.street",
              "case when c.taxnumber is null then '' else c.taxnumber end as taxnumber"],
            from:"customer c",
    ...
    usergroup: {
      ...
      reportkey: () => {
        const sql = {select:["id","reportkey"], from:"ui_report", order_by:["reportkey"]}; 
        return sql;},
      
      menukey: () => {
        const sql = {select:["id","menukey"], from:"ui_menu", order_by:["menukey"]}; 
        return sql;},
