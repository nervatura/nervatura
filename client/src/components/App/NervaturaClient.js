  ...
  _protector(){
    ...
    if(data[APP_MODULE.LOGIN].data){
      ...
      <div theme="${current.theme}" class="main">
        ...
        ></client-edit>` : nothing)}
        ${cache((current.module === APP_MODULE.SETTING) ? html`<client-setting
          id="setting" .data=${data[APP_MODULE.SETTING]} 
          side="${current.side}"
          .auditFilter="${data[APP_MODULE.LOGIN].data.audit_filter}"
          username="${data[APP_MODULE.LOGIN].username}"
          paginationPage=${this.app.getSetting("paginationPage")}
          .onEvent=${this.app.modules.setting}
          .msg="${this.app.msg}" .forms="${{...this.app.modules.forms}}"
        ></client-setting>` : nothing)}
