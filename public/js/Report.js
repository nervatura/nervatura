...
window.Report = function(orientation, unit, format) {
  ...
  var doc = new window.jsPDF({
    orientation: self.orientation, unit: self.unit, format: self.format,
          ...
          for(var index = 0; index < Object.keys(foo_item.columns).length; index++) {
            ...
            switch(etype) {
              case "cell":
                ...
                break;
              case "barcode":
                var code_type = get_value(element["code-type"],"CODE_39");
          ...
          if (img_src !== null) {
            ...
            if (img_src.toString().substr(0,10)==="data:image") {
              ...
              if (check_page_break(self.template.page.cursor.y, img_height)) {
                doc.addPage();}
              doc.addImage(img_src, "", self.template.page.cursor.x, self.template.page.cursor.y+1.5, width, img_height, null, compression);
